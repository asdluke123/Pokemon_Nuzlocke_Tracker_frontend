    ...
    return(
        <div className="run">
            <div className="runTitleArea">
                {social ? <h2>{run.userId.name}</h2>:<div></div>}
                    {social ?<h2>{run.name}</h2> :<h2 onClick={()=>toRunDetail(run.id)} className='runName'>{run.name}</h2>}
            </div>
            <div className="knowledge">
            {social ? run.isComplete ? <h3 className="socailComplete">Complete</h3>:<h3 className="socailComplete">Not finished</h3>: (<div className="checkArea"><p>Complete:</p> <input type="checkbox" onChange= {(e) => setComplete(e,run.id,index)}></input> </div>)}
